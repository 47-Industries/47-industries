  ...
  async function connectMotorev() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const motorevConnectUrl = `https://motorev.app/connect-47i?token=${encodeURIComponent(data.token)}&state=${encodeURIComponent(session?.user?.id || '')}&redirect_uri=${encodeURIComponent(window.location.origin + '/api/account/motorev/callback')}`
