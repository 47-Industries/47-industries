...
export async function GET(req: NextRequest) {
  try {
      ...
      try {
        ...
        await prisma.clientActivity.create({
          data: {
            clientId: client.id,
            type: 'INVOICE_SENT',
            description: `Monthly invoice ${invoiceNumber} generated and sent ($${totalMonthly.toFixed(2)})`,
            metadata: {
        ...
        if (resend && recipientEmail) {
          try {
            ...
            console.log(`  - Sent invoice ${invoiceNumber} to ${recipientEmail}`)
          } catch (emailError) {
        ...
        console.log(`+ Created invoice ${invoiceNumber} for ${client.name} ($${totalMonthly.toFixed(2)})`)
      } catch (clientError) {
    ...
    return NextResponse.json({
      success: true,
      message: `Generated ${results.invoicesCreated} invoices`,
      results,
    })
  } catch (error) {
